...
public class Square : MonoBehaviour
{
	...
	void Update()
	{
		if (thisName == "boxCatapult" )
		{
			...
			{
                DrawLine.aim = true;
                Ball b = busy.gameObject.GetComponent<Ball>();
                if (b.itemKind.color == ItemColor.GREEN)
                {
                    DrawLine.linenumber = 0;
                    linecurrent = 0;
                }
                if (b.itemKind.color == ItemColor.BLUE)
                {
                    DrawLine.linenumber = 1;
                    linecurrent = 1;
                }
                if (b.itemKind.color == ItemColor.VIOLET)
                {
                    DrawLine.linenumber = 2;
                    linecurrent = 2;
                }
                if (b.itemKind.color == ItemColor.ORANGE)
                {
                    DrawLine.linenumber = 3;
                    linecurrent = 3;
                }
                if (b.itemKind.color == ItemColor.RED)
                {
                    DrawLine.linenumber = 4;
                    linecurrent = 4;
                }
                if (b.itemKind.color == ItemColor.YELLOW)
                {
                    DrawLine.linenumber = 5;
                    linecurrent = 5;
                }
            ...
            {
                DrawLine.aim = false;
                DrawLine.instance.LineOverLap[linecurrent].gameObject.GetComponent<LineRenderer>().enabled = false;
                DrawLine.instance.LineOverLap[linecurrent].gameObject.SetActive(false);
            }
		}
